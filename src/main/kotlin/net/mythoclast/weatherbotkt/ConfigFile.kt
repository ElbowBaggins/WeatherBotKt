...
internal object ConfigFile {
  ...
  private fun readFile(): JsonObject {
    try {
      return JsonParser().parse(String(Files.readAllBytes(File("config.json").toPath()))).asJsonObject
    } catch (e: IOException) {
      Logger.getLogger("WeatherBot")
          .severe("Could not access configuration, or configuration is malformed.")
      exitProcess(-1)
    }
  }
}
